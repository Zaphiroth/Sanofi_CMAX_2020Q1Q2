...
# IMS info
ims.prod.part <- fread("02_Inputs/cn_prod_ref_201912_1.txt") %>% 
  setDF() %>% 
  mutate(Pack_Id = str_pad(Pack_Id, 7, "left", pad = "0")) %>% 
  select(Pack_Id, NFC123_Code)

ims.mol.raw <- read.xlsx("02_Inputs/ims_chpa_to19Q4.xlsx")

ims.mol1 <- ims.mol.raw[, 1:21] %>% 
  distinct() %>% 
  filter(!is.na(Pack_Id)) %>% 
  left_join(ims.prod.part, by = "Pack_Id") %>% 
  select(packid = Pack_ID, Corp_ID, Corp_Desc, MNF_TYPE, MnfType_Desc, 
         Mnf_Desc, ATC4_Code, NFC123_Code, Prd_desc, Pck_Desc, 
         Molecule_Desc)

ims.prod <- fread("02_Inputs/cn_prod_ref_201912_1.txt") %>% 
  setDF() %>% 
  mutate(Pack_Id = str_pad(Pack_Id, 7, "left", pad = "0"))

ims.mol.lkp <- fread("02_Inputs/cn_mol_lkp_201912_1.txt") %>%
  setDF() %>%
  arrange(Pack_ID, Molecule_ID) %>%
  mutate(Pack_ID  = str_pad(Pack_ID , 7, "left", pad = "0"))

ims.mol <- fread("02_Inputs/cn_mol_ref_201912_1.txt")

ims.corp <- fread("02_Inputs/cn_corp_ref_201912_1.txt")

ims.mol2 <- ims.mol.lkp %>%
  left_join(ims.mol, by = c("Molecule_ID" = "Molecule_Id")) %>%
  arrange(Pack_ID, Molecule_Desc) %>%
  group_by(Pack_ID) %>%
  summarise(Molecule_Desc = paste(Molecule_Desc, collapse = "+")) %>%
  ungroup() %>%
  left_join(ims.prod, by = c("Pack_ID" = "Pack_Id")) %>%
  left_join(ims.corp, by = "Corp_ID") %>% 
...
ims.mol <- ims.mol2 %>% 
  ...
  bind_rows(ims.mol1) %>% 
  select(packid, atc4 = ATC4_Code, nfc = NFC123_Code, molecule = Molecule_Desc, 
         product = Prd_desc)


##---- Formatting raw data ----
# Guangzhou
raw.gz1 <- read.csv('02_Inputs/data/gzs 20q1_date.csv')
...
raw.gz <- raw.gz1 %>% 
  ...
  select(year, month, quarter, province, city, district, pchc, atc4, nfc, molecule, 
         product, packid, price, units, sales)
